...
object Framework {
  ...
  /**
    ...
    */
  implicit def rxMod[T <: dom.html.Element](r: Rx[HtmlTag])(implicit ctx: Ctx.Owner): Frag = {
  ...
  implicit def RxAttrValue[T: AttrValue](implicit ctx: Ctx.Owner) = new AttrValue[Rx.Dynamic[T]] {
    def apply(t: dom.Element, a: Attr, r: Rx.Dynamic[T]): Unit = {
      r.trigger {
  ...
  implicit def RxStyleValue[T: StyleValue](implicit ctx: Ctx.Owner) = new StyleValue[Rx.Dynamic[T]] {
    def apply(t: dom.Element, s: Style, r: Rx.Dynamic[T]): Unit = {
